              ...
              <div className={cn(
                ...
                <div>
                  <p className={cn("text-[11px] font-bold",
                    ...
                    {realAlloc.obligPct >= 100 ? "Alerta: Obligaciones superan ingresos" :
                     realAlloc.obligPct >= 70 ? "Navegando con poco margen" : "¡Vas por buen camino! 🌿"}
                  </p>
                  <p className={cn("text-[10px] mt-0.5", realAlloc.obligPct >= 100 ? "text-red-600/80" : "text-muted-foreground")}>
                    {realAlloc.obligPct >= 100 ? `${Math.round(realAlloc.obligPct)}% de carga.` :
                     realAlloc.obligPct >= 70 ? `El ${Math.round(realAlloc.obligPct)}% va a obligaciones.` :
                     "Distribución inteligente activa."}
                  </p>
                </div>
              </div>
            </CardContent>
                  ...
                  <CardContent className="p-4 space-y-2">
                    ...
                    <p className="text-[11px] text-muted-foreground leading-relaxed">
                      {recommendations.strategies.avalancheWins
                        ? `Ataca primero "${recommendations.strategies.avalanche.pasos[0]?.nombre ?? "tu deuda más pesada"}".`
                        : `Empieza por "${recommendations.strategies.snowball.pasos[0]?.nombre ?? "tu deuda más pequeña"}".`}
                    </p>
                  </CardContent>
                </Card>
              )}
